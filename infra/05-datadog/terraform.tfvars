...
# 평가 창과 집계 — 2026-08-26
#
# 라이브커머스라 조치가 빨라야 한다. 시간이 어디로 가는지 재 보니(M-020) 이렇다.
#
#   모니터 평가(희석 + 주기)   ~4분    ← 여기만 우리가 줄일 수 있다
#   Datadog -> Queue 전송      63~68초  실측. 못 줄인다
#   Correlator + Worker        12초     길 A 로 바꿔 없앴다
#
# **창을 1분으로 줄이고 집계를 max 로 바꿨다.** 둘 다 같은 문제를 푼다 —
# `avg(last_5m)` 은 나쁜 구간이 좋은 구간에 섞여 임계를 늦게 넘는다. 평시 8ms 가
# 900ms 로 급변해도 5분 평균은 T+1분에 186ms 다. `max` 는 창 안에 한 점이라도
# 넘으면 즉시 운다.
#
# **10초는 못 한다.** Datadog metric monitor 의 최소 창이 1분이고 평가 주기도
# 약 1분이다(variables.tf 의 validation). 바닥은 약 2분이다.
#
# 대가는 스파이크 오탐이다. S1 조치는 L3(사람 승인)이라 잘못 깨워도 사람이
# 거부하면 끝이지만, **S2 의 1차 조치는 L1(자동 실행)이다** — 스파이크 하나로
# 파드가 늘어날 수 있다. S2 실험에서 그 빈도를 보고 필요하면 S2 만 avg 로
# 되돌린다.
###############################################################################
scenario_entry_window_minutes = 1
scenario_early_window_minutes = 1
