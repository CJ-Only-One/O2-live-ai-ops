...
| **클러스터 사양** | 결정 당시 노드그룹은 `t3.small` × 2 (max 3) 였고 Dify 는 컨테이너 9개에 실사용 8 GiB 였다 — 전용 노드그룹을 새로 파야 했다. 지금은 `c6i.large` × 4 (max 6) 지만 결정을 되돌리지 않는다. 근거는 위의 **블래스트 반경**이지 사양이 아니다 |
...
| `agent_fixed` | 에이전트가 런북을 실행해 해결 | 실행기는 생겼다(S1·S2·S3). 이 값을 자동으로 쓰지는 않는다 — 사람이 확인하고 표시한다 |
| `false_alarm` | 오탐. 장애가 아니었다 | 사람 |

★ **자동 기록은 절대 `human_fixed` 로 가지 않는다.** 지표가 돌아온 것과 사람이
고친 것은 다르고, 이걸 해결로 적으면 발표의 MTTR 숫자가 거짓이 된다. 사람이
조치했다면 사람이 그렇게 표시한다(`scripts/verify.py`).

`action_taken` 은 더 이상 `"none"` 고정이 아니다 — 워크플로가 조치까지 하게 된
뒤 `worker.py` 의 `_last_action_taken()` 이 `attempt_log` 마지막 항목의 실제
`action_id` 를 넣는다. 진단만 하고 끝난 실행에서만 `"none"` 이다. 공통 진입점
Worker(`agent_entry_worker.py`)는 아직 `"none"` 고정이다.
