...
resource "aws_lambda_function" "worker" {
  ...
  # 2026-08-23 Shadow E2E에서 5초 cold invocation이 timeout 났다(T-020, M-011).
  # 10초는 실측 5초보다 여유를 두되 Queue visibility 30초보다 짧게 유지한다.
  timeout = 10

  # SQS poller가 예약 동시성보다 많은 batch를 먼저 받아 visibility 동안 묶지 않게
  # event source의 maximum_concurrency와 같은 값으로 둔다(T-020).
