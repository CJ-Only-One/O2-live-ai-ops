...
| 2026-08-21 | Worker (이력 추가 후) | VPC 안 | 4,471ms | 240ms | **93MB** |
...
| 구간 | 시간 | 비고 |
|---|---|---|
| 임베딩 + 벡터 검색 | 1,317ms | **검색이 권한 오류로 실패한 회차다**(T-015). 정상 회차는 더 걸린다 |
| Dify 왕복 | 518ms | `elapsed_time` 0.166s + 네트워크 |
| S3 + S3 Vectors 저장 | 513ms | |
