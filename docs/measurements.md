...
| `api`·`order-worker` 가 무부하에서 1 vCPU 를 쓰는 이유 | M-008 참조. 노드 CPU 크레딧이 0 이라 **부하를 걸기 전부터 baseline 으로 제한된 상태**다. 원인을 모르면 부하 테스트 결과가 앱 성능이 아니라 스로틀링을 재게 된다 | 프로파일러를 붙이거나, 두 서비스의 공통 요소(이벤트 SDK·Datadog 트레이서)를 하나씩 빼며 본다 |
...
| 날짜 | 파드 | CPU | 메모리 |
|---|---|---|---|
| 2026-08-20 | api | **987m** | 131 MiB |
| 2026-08-20 | order-worker | **955m** | 98 MiB |
...
Datadog 트레이서를 공유하므로 공통 요소를 의심한다. M-007 에 항목을 남겼다.

**부하 테스트 전에 이것부터 풀어야 한다.** 크레딧이 0 인 노드는 baseline
으로 스로틀링되므로, 지금 부하를 걸면 앱 성능이 아니라 스로틀링을 재게 된다.
