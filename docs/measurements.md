...
`o2-eks`, `t3.small` × 3 (당시 노드그룹 `max_size = 3` 이라 **그때는 최대치**),
2026-08-20 17:40 KST. **노드는 이후 `c6i.large` × 4(max 6)로 바뀌었다** — 아래
2026-08-21 이후 행과 `infra/02-eks/terraform.tfvars` 를 본다. `kubectl top` 값이며 요청량이 아니라 **실사용량**이다.
...
### 요청 할당 여유 — canary 를 얹을 자리가 있는가 (2026-08-24)

**실사용량이 아니라 requests 할당량이다.** 스케줄러가 보는 값이라 "파드를 하나 더
넣을 수 있는가" 는 이쪽으로 정해진다. 위 절들의 `kubectl top` 값과 섞지 않는다.

**조건 (2026-08-24)** — `c6i.large` × 2, allocatable 노드당 1,930m / 3,140Mi.
o2-dev 파드 8개(api 2 · chat-gateway 2 · frontend · mediamtx · order-worker 등),
방송·부하 없음. `kubectl describe node` 의 `Allocated resources`.

| 날짜 | 노드 | CPU requests | 메모리 requests | CPU 여유 |
|---|---|---|---|---|
| 2026-08-24 | ip-10-0-153-138 | 1,350m (69%) | 2,148Mi (68%) | **580m** |
| 2026-08-24 | ip-10-0-66-38 | 1,380m (71%) | 2,704Mi (86%) | **550m** |

**해석 — api 자원 요청을 500m 으로 올리면 canary 자리가 사라진다.**

api 는 requests `cpu: 100m` 이고 M-009 실측은 300 RPS 에서 664m 이라 요청값이
실사용의 1/6 이다. 그런데 2파드를 500m 으로 올리면 +800m 이 필요하고, 남는
여유는 노드당 100m 남짓이 된다. **S2 의 canary 는 세 번째 api 파드다** —
들어갈 자리가 없어 Karpenter 가 노드를 새로 띄운다. 실험 중 노드가 늘면
프로비저닝 타이밍이 실행마다 달라진다(D-051, `scenario-experiment.md` 3절).

그래서 자원 요청 상향은 보류다. **재개 조건 — api 에 HPA 를 붙이기로 하거나,
노드 여유가 1파드분 이상 늘어나면 그때 올린다.** 판단 근거가 이 표다.

메모리 쪽은 `ip-10-0-66-38` 이 86% 라 더 빠듯하다. 384Mi 를 요청하는 파드가
그 노드에는 하나만 더 들어간다.

**다시 재야 할 때** — replicas·requests 를 바꿀 때, 노드 타입·대수를 바꿀 때,
DaemonSet 이 늘 때. 값이 스케줄링 결정에 직접 쓰이므로 낡으면 조용히 틀린다.
...
| 2026-08-25 | 2,000 | 5/s | 10,000 | 5,939 | 425ms | 555ms | 844m | 144Mi | 99.8% |
| 2026-08-25 | 2,000 | 10/s | 20,000 | 6,766 | 653ms | 985ms | 1,016m | 152Mi | 99.8% |
| 2026-08-25 | 2,000 | 15/s | 30,000 | 7,637 | 511ms | 735ms | 1,187m | 146Mi | 99.9% |
| 2026-08-25 | **2,000** | 20/s | **40,000** | 7,465 | **1,252ms** | **7,302ms** | 1,277m | 154Mi | **99.3%** |

**조건 (2026-08-25)** — S1 붕괴점을 좁히려고 시청자를 2,000 으로 고정하고 채팅율만
올렸다. chat-gateway 이미지는 `379e4c1`(방송 축 태그 추가본), 파드는 서로 다른 노드,
`api-canary` 없음, 방송 `bc_s1meas`, 계단당 램프 30초 · 유지 180초.

전 구간에서 **연결 실패 0 · 조기 종료 0 · 깨진 프레임 0** 이다. 무너질 때도 죽지 않고
느려지기만 했다.

**2026-08-25 관측 1 — 이 표의 전파 p95 는 서버 지연이 아니다. 붕괴점 판정에 쓰지 마라.**

같은 시각 **서버측** `o2.chat.propagation` p95 는 38,526 아이템/s 에서 **186ms**,
제한 적용 구간에서도 145~220ms 였다. 같은 부하에서 이 표의 k6 값은 1,252ms 다. **6.7배 차이다.**

두 값이 재는 구간이 다르다. 서버측은 발행 시각부터 WebSocket send 직전까지고,
k6 값은 거기에 네트워크와 **k6 자신의 이벤트 루프 지연**을 더한 것이다. 노트북 하나가
2,000 소켓에 초당 7,500 프레임을 받으면 그 밀림이 서버 지연으로 기록된다 — 이 문서
아래 "프레임/s 가 예상보다 적으면" 주석과 같은 현상이고, 이번에 그 크기를 실제로 쟀다.

따라서 **40,000 에서 무너진 것은 서버가 아니라 부하 생성기 쪽**이다. 전달률 99.3%
하락도 같은 원인으로 본다. 8월 21일 값들도 전부 k6 클라이언트측이므로 같은 편향을
포함한다 — **M-010 의 "2 파드 안전선 20,000 아이템/s" 는 서버 용량이 아니라 이 측정
방식의 한계선일 수 있다.** 서버 용량을 다시 정하려면 클러스터 밖 부하 생성기를
여러 대로 나누거나 서버측 지표로만 판정해야 한다.

**2026-08-25 관측 3 — 채널 총량 제한(S1 조치)의 단위와 차단률을 확인했다.**

같은 부하(2,000 연결 · 채팅율 20/s = **분당 1,200 건**)에서 `cfg:channel_limit:bc_s1meas`
를 900 → 600 → 300 으로 내리며 관찰했다.

| 확인한 것 | 결과 |
|---|---|
| 제한값의 단위 | **분당 방송 전체 발화 건수**. `chat:total:{방송}` 이 `INCR` + `EXPIRE 60` 이다 |
| 카운터 거동 | 매 분 0 에서 시작해 상한을 넘긴 뒤로 그 분이 끝날 때까지 계속 거부한다. `LIMIT=600` 이면 분당 30 초 지점에서 넘어간다 |
| 차단률 | `LIMIT=600` 구간에서 **0.49**. 산식 `1 - LIMIT/분당발화` = 0.5 와 일치한다 |
| 원복 | 해제 직후 팬아웃이 41,787 아이템/s 로 복귀. 지연 재발 없음(227ms) |
| 서버측 전파 p95 | 제한 없음 186ms, 제한 구간 145~220ms, 해제 후 227ms |

**런북의 `limit=500` 과 차단률 상한 `0.05` 는 서로 모순이다.** 이 부하에서 500 은
`1 - 500/1200` = **58% 차단**이라 상한을 열 배 넘긴다. 둘 중 하나는 실측으로 다시
정해야 한다. 강도별 회복 곡선은 아래 관측 4 때문에 아직 못 만들었다.

**2026-08-25 관측 4 — 서버측 지표 조회가 창 끝에서 틀린 값을 낸다. 판정에 그대로 쓰면 안 된다.**

Hot 논리 지표 Adapter 의 `_latest()`(`o2hot/metric_catalog.py:227`)는 창 안에서
타임스탬프가 가장 큰 점 **하나**를 값으로 쓴다. Datadog 의 마지막 버킷은 아직 집계
중이라, 차단이 실제로 일어나는 동안에도 다음이 관측됐다.

| 지표 | 관측값 | 실제 |
|---|---|---|
| `items_per_sec` | `0.0` (`sample_count` 은 1,193,650) | 팬아웃 동작 중 — WebSocket 프로브가 15 초에 196 아이템 수신 |
| `block_rate` | `0.0` → `1.0` | 0.49 (카운터 산식과 일치하는 순간도 있었다) |

`block_rate` 는 같은 시각에 0 과 1 을 오갔다. warm `latest` 창도 같은 순간
`channel_limited_rate = 0` 을 줬다 — 방금 열린 창을 최신으로 주기 때문이다.

**S1 복구 판정이 이 값을 읽는다.** `items_per_sec ≤ 20,000` 은 0 이 오면 무조건
통과하고, `block_rate ≤ 0.05` 는 1.0 이 오면 무조건 실패한다. 잡음이 아니라 판정을
뒤집는다. 같은 `_latest()` 를 `latency_p95` · `failure_rate` · `cache_hit_rate` 도
쓰므로 S2 · S3 판정도 같은 위험을 진다. 비율 지표는 분자 · 분모를 창 전체로 합산해
나누고, 게이지 · 백분위는 완성된 버킷만 써야 한다.

**2026-08-26 — 서버측 지표로만 다시 쟀다. 한계는 CPU 가 아니라 메모리다.**

2026-08-25 관측 1 에서 이 표의 k6 값을 못 쓴다고 판단해, 판정 지표를 서버측
`o2.chat.propagation` 하나로 고정하고 연결 수를 계단으로 올렸다
(`loadtest/s1-collapse.sh`). 계단마다 램프 30초 · 유지 420초, 마지막 120초만 읽었다.
`api-canary` 없음, 방송 `bc_s1meas`, chat-gateway 2 replicas, 이미지 `379e4c1`.

| 연결 | 채팅율 | 아이템/s | 파드당 연결 | 서버 p95 | cg CPU(2파드) | 파드당 MEM | 결과 |
|---|---|---|---|---|---|---|---|
| 2,000 | 20/s | 40,000 | 1,000 | 227ms | 1,181m | 76Mi | 정상 |
| 3,000 | 20/s | 60,000 | 1,500 | 227ms | 1,738m | 87Mi | 정상 |
| 4,000 | 20/s | 80,000 | 2,000 | 325ms | 2,115m | ~110Mi | 정상 |
| **6,000** | 20/s | **120,000** | 3,000 | **576ms** | 2,271m | **241Mi** | **열화 · 안정** |
| 8,000 | 20/s | 160,000 | 4,000 | ~700ms | ~2,330m | **377Mi** | **OOMKill** |

전 구간에서 k6 임계 초과는 없었다 — 부하 생성기는 8,000 연결까지 멀쩡했다.
따라서 이 표의 p95 는 서버 값이다.

**관측 1 — M-010 의 "2 파드 안전선 20,000 아이템/s" 는 서버 용량이 아니었다.**

서버는 80,000 까지 325ms 로 버틴다. 8월 21일 표에서 40,000 을 붕괴로 읽은 것은
k6 클라이언트측 계측 편향이다(T-041). 안전선은 최소 4 배 위에 있다.

**관측 2 — 먼저 닿는 벽은 메모리다.**

160,000 에서 CPU 는 파드당 약 1,165m 로 노드(c6i.large, 2 vCPU)의 58% 였는데
메모리가 384Mi limit 에 닿아 OOMKilled 됐다. 팬아웃 드롭은 0 이었다 — 버린 것이
아니라 `conn.pending` 에 쌓이면서 밀린 것이고, 그 큐가 곧 메모리다. 지연이 오르기
시작하면 메모리도 같이 오르므로 **열화와 파드 사망이 같은 축에 있다.**
`CHAT_MAX_PER_TICK` 이 연결당 상한이라 연결 수가 늘면 총량이 그만큼 늘어난다.

**관측 3 — 알림은 뜬다. 다만 창과 집계 방식이 판정을 가른다.**

측정 당일 `[O2][S1] Chat 전파 p95 지연` 은 `max(last_1m) >= 800`(warning 400) 으로
바뀌어 있었다. 6,000 연결 구간에서 이 Monitor 가 실제로 `Alert` 로 전이했다 —
**창 평균은 576ms 인데 순간 최댓값은 800ms 를 넘는다**는 뜻이다. 같은 부하를
`avg(last_5m)` 로 보면 안 울린다. 창 길이와 집계 함수가 곧 판정 기준이므로
임계값만 보고 "닿는다/안 닿는다"를 말할 수 없다.

진입 evidence 인 `[O2][S1] 채팅 팬아웃 총량` 은 `avg(last_1m) >= 30,000/s`
(warning 20,000) 이라 시연 부하 120,000 아이템/s 에서 네 배로 넘긴다.

반면 런북 `chat_channel_overload` 의 성공 기준 `chat_propagation_p95_ms <= 800` 은
Hot 논리 지표(창 전체 집계) 를 읽으므로 정상(227ms) 이든 열화(576ms) 든 항상 참이다.
**Monitor 는 순간 최댓값으로 울리고 복구 판정은 창 평균으로 재는 비대칭**이라,
"알림이 뜬 상태" 와 "복구 기준 미달" 이 서로 다른 축에서 판정된다. 성공 기준은
실측 기반으로 다시 정해야 한다 — 정상 227ms 와 열화 576ms 사이 어딘가다.

**시연 부하는 120,000 아이템/s(6,000 연결 × 20/s)다.** 정상 대비 p95 가 2.5 배로
오르고, 메모리는 limit 의 63% 에서 멈추며, 파드가 죽지 않아 조치와 복구 검증이
성립한다.

**2026-08-25 관측 2 — CPU 는 8월 21일보다 2.7배 나빠졌다. 이건 서버측 값이다.**

`2,000 연결 · 40,000 아이템/s` 에서 467m 이던 것이 **1,277m** 가 됐다. `kubectl top`
으로 잰 서버측이라 관측 1 의 편향과 무관하다. 오염 요인(canary·타 부하)은 배제했고
api CPU 는 전 구간 6~7m 이었다. 원인은 안 찾았다 — 후보는 `broadcast_id` 태그 추가로
늘어난 DogStatsD 패킷, 28시간 전 들어온 `cue-warmer` 파드, 노드 동거 변화다.
...
### S2 canary CPU 상한 스윕 (2026-08-26)

**cue-warmer 는 켜져 있었다.** 이 절은 파드 자체의 거동만 재므로 Agent 조치가
개입하지 않았고(부하 60초가 알림→조치 약 2분보다 먼저 끝난다), 매 지점 전에
`api replicas=2` 를 확인했다. Agent 를 포함한 측정은 워머를 세우고 해야 한다(T-043).

**조건** — 08-25 리허설과 같은 프로필. ALB 경유 `GET /api/broadcasts/bc_1042`,
constant-arrival-rate **200 RPS · 60초**, k6 `preAllocatedVUs=maxVUs=1200`,
`noConnectionReuse=true`. canary readiness 5초 · failureThreshold 6,
liveness timeout 5초. main 2 + canary 1.

| CPU 상한 | p95 (k6) | 실패율 | dropped | canary 재시작 | 판정 |
|---|---:|---:|---:|---:|---|
| `100m` | 32.16s | 3.00% | 746 | **1** | 파드가 죽는다 · 생성기도 포화 |
| `125m` | 17.30s | 0% | 0 | 0 | |
| `125m` (재현) | 22.30s | 2.58% | 0 | 0 | |
| `150m` | 10.97s | 1.11% | 0 | 0 | 장애가 약해진다 |

**`125m` 으로 정한다.** 재시작이 없는 값 중 장애가 제일 강하다.

**`100m` 은 못 쓴다.** liveness timeout 을 5초로 넓힌 뒤에도 재시작이 났다 —
`Liveness probe failed: context deadline exceeded (x3 over 87s)` 로 kubelet 이
컨테이너를 죽인다. 파드가 Service 에서 빠졌다 들어오며 지연이 요동치고, 같은 실행에서
생성기까지 포화(dropped 746, 실제 175/s)돼 값을 믿을 수 없다.

**p95 는 실행마다 흔들린다** — 같은 `125m` 에서 17.3s 와 22.3s, 실패율 0% 와 2.58%.
장애 강도를 점 하나로 고정할 수 없다. 흔들리지 않는 것은 셋이다: 재시작 0,
중앙값 16ms 대, 임계 800ms 를 매번 크게 초과. **판정에는 이 셋을 쓰고 p95 절대값은
쓰지 않는다.**

**`125m` 의 재시작 0 은 60초 부하에서만 성립한다.** 같은 canary 로 200 RPS 를
**15분** 연속으로 걸었더니 재시작이 **3회** 났다(2026-08-26 23:12~23:27). 위 표는
전부 60초 실행이다. 스로틀 아래서 probe 실패가 누적되는 구조라 **부하 길이가
길수록 재시작 확률이 오른다** — 시연처럼 몇 분 이상 끌 계획이면 `150m` 쪽이나
liveness `failureThreshold` 확대를 다시 재야 한다. **지속 부하에서의 재시작
0 지점은 아직 안 쟀다.**

**파드별 지연 (서버측, `125m` 재현 실행 직후 Warm 스냅샷)**

| 파드 | p95 | 표본 |
|---|---:|---:|
| `api-6b6bffd4df-lgtkv` | 12.38ms | 744 |
| `api-6b6bffd4df-dmmt9` | 12.38ms | 747 |
| `api-canary-6bfc57557d-f4rk4` | **100.70ms** | 208 |

서비스 전체는 `p50 3.06ms · p95 28.63ms · p99 100.7ms`. **canary 가 정상 파드의
8.1배**이고 정상 파드 둘은 같다 — S2 재진단이 쓸 증거가 실제로 채워진다.
`pod_latency_outlier_tolerance = 2.5` 로도 걸린다.

canary 표본이 정상 파드의 1/3.6 인 것은 느려서 같은 시간에 덜 처리하기 때문이다.

k6 p95(초 단위)와 서버측 p95(밀리초 단위)의 차이는 T-041 이 기록한 클라이언트측
계측·생성기 포화 문제다. **파드 간 비교는 서버측 값으로만 한다.**
...
### S2 자동 Agent E2E — 어디까지 갔고 무엇이 막았나 (2026-08-26~27)

**이 절은 조치 효과가 아니라 "파이프라인이 어디서 끊겼나"의 기록이다.** 같은 부하
프로필(ALB 경유 `GET /api/broadcasts/bc_1042`, 200 RPS, k6 `preAllocatedVUs=maxVUs=1200`,
`noConnectionReuse=true`)로 12회 돌리며 막는 것을 하나씩 걷어냈다. 조치 수치는
`S2 실제 부하와 2단계 조치 리허설` 절을 본다.

**도달 구간**

| Phase | 내용 | 결과 |
|---|---|---|
| 1 | Datadog p99 진입 → Worker → History `matched 0` | **재현됨** |
| 2 | 용량 가설 오진 → 범용 런북 → `api` 2 → 3 증설 | **재현됨** |
| 3 | 검증 실패(`NO_RECOVERY`) → `unresolved` | **재현됨** |
| 4 | 재진단 라운드 진입 | **열림**(loop 수정 후) |
| 4b | `pod_load_skew` 로 정정 | **안 됨** |
| 5·6 | 격리 · 증설분 원복 · `RESOLVED` | 미도달 |

**걷어낸 것 여섯 — 전부 실측으로 원인을 확정한 뒤 고쳤다**

| # | 증상 | 실제 원인 | 어디를 고쳤나 |
|---|---|---|---|
| 1 | 증설이 3~5초 만에 원복 | `cue-warmer` 가 `api` 의 `spec.replicas` 를 10초마다 폴링해 기준값으로 되돌린다. 누가 바꿨는지 구분하지 않는다 | 실험 중 워머 정지 절차(T-043) |
| 2 | 워크플로가 매번 400 으로 사망 | 서비스 단위 알림이라 방송 축이 없어 `broadcast_id` 가 `LIVE-001` 로 fallback, `22-I` 노드가 400(T-044) | 진입 Monitor 에 `broadcast_id` 태그 |
| 3 | 조치 한 번에 `auto_recovered` | 판정이 `p95_ms <= 350` / `latency_p95 <= 800` 뿐이라 Warm 서버측 밀리초 값(장애 중 28.6ms)이 그냥 통과 | 두 용량 런북에 `p99_ms <= 50` |
| 4 | 1차 진단부터 `pod_load_skew` 직행 | 진입 알림 본문에 원인·오진 함정·다음 조사 방향이 다 적혀 있었다. 알림은 진단 프롬프트로 그대로 들어간다 | 본문을 사실 한 줄로 |
| 5 | 재진단 라운드가 아예 없음 | 라이브 워크플로의 `main_loop.loop_count` 가 **1**(git 기록본과 노드 제목은 10) | Dify 에서 10 으로 |
| 6 | 격리해도 `unresolved` | `pod_load_skew` 런북의 `overall_failure_rate <= 0.01` 이 Warm 의 `null` 을 만나 구조적으로 통과 불가 | 조건 제거 + `p99_ms <= 50` |

**5번을 어떻게 확정했나** — 실행 기록이 갈랐다. `state_reducer` 가
`next_stop_flag=false / final_status=RUNNING / remediation_retry=1 / skip_diagnosis=true`
를 내놓는데(= "재조치하라") 루프가 1회만 돌고 종료했다. 중단 조건(`stop_flag=="true"`)은
한 번도 충족되지 않았다. 결정타는 **성공한 실행 6개 전부 `state_reducer`·`diagnosis_agent`·
`action_http` 가 각 1회**였다는 것이다 — 2회 돈 실행이 하나도 없었다. 고친 뒤 실행
(`771ef057-da8c-4997-b385-966fae688fc4`)에서 `state_reducer` 3회 · `diagnosis_agent` 2회로
바뀌었다.

**남은 하나 — 재진단이 같은 답을 낸다**

고친 뒤에도 1차·2차 진단이 모두 `traffic_spike_overload`(confidence **둘 다 0.65**)라
`ACTIONS_EXHAUSTED_SAME_RCA` 로 끝난다. 상태 머신의 그 판단 자체는 옳다.

**데이터 부족이 아니다.** 파드별 지연은 두 프롬프트에 다 들어 있다.

| | 1차 | 2차 |
|---|---|---|
| 프롬프트 길이 | 22,507자 | 26,517자 |
| `latency_p95_by_pod` 위치 | 3,012자 | 3,120자 |
| 같은 키 반복 횟수 | **7회** | **7회** |
| 검증 결과(`NO_RECOVERY`) 위치 | 없음 | 22,147자 (**맨 끝**) |

실제 값도 선명하다 — canary 202.54ms 대 정상 파드 5.35·6.15ms(**33배**), 표본 601건.

즉 **1차와 2차가 사실상 같은 것을 본다.** `windows` 6개와 `latest` 에 걸쳐 7번 반복되는
중첩 dict 라 비교가 어렵고, 재진단의 근거가 되어야 할 "증설했는데 p99 가 그대로"는
26,517자 중 22,147자 지점, 즉 스냅샷 뒤에 덧붙어 있다. 회차가 달라도 입력이 안 달라지니
confidence 까지 동일하게 나온다.

**고칠 방향** — 노드 7(`context_enrich`)을 회차별로 나눈다. 1차에는 파드별 분해를 빼고
(서비스 전체 `latency_p50/p95/p99` 만 남긴다 — 이 값이 이미 모든 파드를 합친 값이라
파드 하나가 튄 건지 전체가 느린 건지 구분되지 않는다), 재진단에는 파드별 값과
`worst/median 비율`을 앞쪽에 둔다. 회차는 `state_read.diagnosis_retry` 로 안다.

### 안 잰 것 (이 절)

- **`p99_ms <= 50` 의 통과쪽 근거.** 미달쪽은 실측이다(장애 중 100.7ms, 무부하 2.01ms).
  격리 후 p99 는 아직 못 쟀다 — 격리까지 도달하면 그 값으로 갱신한다.
- **canary 표본 수의 CPU 상한 의존성.** `125m` 에서는 canary 표본이 0~2건이라 그 파드의
  p95 자체가 계산되지 않았고, `150m` 에서는 203~675건으로 값이 나왔다. 어느 지점부터
  표본이 충분해지는지는 두 점만 있고 사이를 안 쟀다.
