...
**켠 첫 판에 하나가 깨졌다** — `test_history.py` 가 `NoRegionError` 로 죽었다.
그 파일은 boto3 가 **없을 때만** 가짜를 끼운다. 로컬에는 boto3 가 없어 늘
가짜가 꼈고, CI 러너에는 **설치돼 있는데 설정이 없다.** 그래서 진짜 boto3 가
쓰였고 `ingress.py` 가 import 시점에 부르는 `boto3.client("lambda")` 가 리전을
못 찾았다. `AWS_DEFAULT_REGION` 을 시험 안에서 기본값으로 넣어 고쳤다 —
클라이언트를 만들 때만 필요한 값이고 AWS 를 부르지는 않는다.
