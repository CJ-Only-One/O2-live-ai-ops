...
**저장소의 서브 Monitor 넷에는 알림 핸들이 없다.** Datadog 은 메시지에 `@`
핸들이 없으면 어디로도 보내지 않는다. 상태는 계산하지만 아무도 안 깨운다.

### 다만 계정은 저장소와 달랐다 — 하나가 울고 있었다

위 표는 **Terraform 코드**를 읽은 것이다. `terraform plan` 을 돌려 계정과
대조했더니 한 건이 어긋나 있었다.

```
# datadog_monitor.latency_p95_high will be updated in-place
  ~ message = <<-EOT
      - 응답 p95 지연 시간이 임계치를 초과했습니다. 복합 모니터의 하위 조건으로 작동합니다.
      - @webhook-o2-dify
      + 응답 p95 지연 시간이 임계치를 초과했습니다. 복합 모니터의 하위 조건으로 작동합니다.
    EOT
```

**누군가 콘솔에서 `@webhook-o2-dify` 를 붙였다.** 즉 서브 Monitor 넷 중
`latency_p95_high` 하나는 **실제로 에이전트를 깨우고 있었다.** F-9 의 걱정은
근거가 없었던 것이 아니라 **대상이 하나였고, 그것도 코드가 아니라 콘솔
편집에서 생긴 것**이다.

이 결정을 바꾸지는 않는다 — 오히려 그대로 apply 하면 핸들이 지워져 코드의
의도가 복원된다. 다만 **왜 이 결정이 옳은지의 근거가 하나 바뀐다.**
"서브는 원래 안 운다" 가 아니라 **"서브는 울면 안 되고, 콘솔 편집이 그
약속을 깼으므로 되돌린다"** 가 맞다.

**그래서 규칙을 하나 덧붙인다 — Monitor 를 콘솔에서 고치지 않는다.**
이 스택은 Terraform 이 소유한다. 콘솔 편집은 다음 apply 에 조용히 지워지고,
지워지기 전까지는 **저장소를 읽어서는 알 수 없는 알림**이 존재한다.
급하면 고치되, 같은 변경을 코드에도 반영하고 PR 을 낸다.

즉 F-9 가 걱정한 "안 쓰는 알림이 같이 울린다" 는 **게이트가 없어서가 아니라
콘솔 편집 때문에** 실재했다. 게이트가 없는 것과 우는 것은 여전히 다른
문제다.

코드 기준으로 게이트 없이 우는 것은 셋이고, 그중 새 명세에 대응이 없는 것은
...
이게 왜 급한가 — 명세의 자기 교정 루프가 **조치 후 90초 뒤 재확인**을
전제한다. 집계가 100초 밀려 있으면 그때 읽는 값은 조치 이전 값이고,
**에이전트가 자기 조치의 효과를 반대로 판정한다.**

구성은 `stream-business` 샤드 1개 · 이벤트 소스 매핑
`parallelization_factor` 1 이라 소비가 사실상 직렬이다. 손잡이는 둘로 보인다.

1. 샤드 수를 늘린다
2. `parallelization_factor` 를 올린다

Confluence 검토 문서도 이 둘을 **"샤드 수 또는 `parallelization_factor`"**
로 나란히 적었다. **둘은 나란히 놓을 수 있는 선택지가 아니다.**

### 2번은 데이터를 조용히 버린다

집계기의 이중 집계 가드가 이렇게 생겼다(`WindowSketch.already_applied`).

```python
for src, hi in other.seq.items():
    cur = self.seq.get(src)
    if cur is not None and int(cur) >= int(hi):
        return True          # 이미 반영된 배치 — 버린다
```

샤드별로 **본 번호의 최댓값 하나**만 들고 있다. 이 방식은 재시도를 걸러내는
데는 충분하지만 **"한 샤드를 한 번에 하나씩 처리한다"** 를 전제한다.

`parallelization_factor` 를 1보다 크게 하면 같은 샤드를 여러 배치가 **동시에**
처리하고, 늦게 시작한 배치가 먼저 끝날 수 있다. 그러면 앞선 배치가 도착했을
때 `cur > hi` 가 되어 **재시도로 오인돼 통째로 버려진다.**

실측했다 — 같은 샤드에서 배치 둘(각 10건)이 뒤바뀌어 도착하면:
...
**예외도 오류도 나지 않는다.** Lambda 는 성공으로 끝나고 `rps` 만 조용히
낮아진다. 이 파이프라인에서 가장 나쁜 실패 모양이다 — 틀린 값이
정상적으로 보인다.

### 1번은 안전하다

`aggregate._source_of()` 가 `source` 키를 **스트림 이름 + 샤드 ID** 로 만든다.

```python
return f"{stream}:{shard}"
```

샤드가 다르면 키가 갈리고, 가드는 키별로 독립적으로 비교한다. 두 샤드의
Lambda 호출이 동시에 돌아도 서로의 번호를 덮어쓰지 않는다.

DynamoDB 쓰기 경쟁도 이미 처리돼 있다 — `WarmStore.merge_sketch()` 가 `rev`
기반 낙관적 잠금으로 쓰고, 충돌하면 다시 읽어 병합한다. 경쟁자가 둘에서
셋으로 느는 것뿐이다.

### 결정

**`parallelization_factor` 는 1로 고정한다. 지연은 샤드 수로 푼다.**

Terraform 쪽 이벤트 소스 매핑 둘(`lambda.tf` · `warm-path.tf`)에 이 값을
올리면 안 되는 이유를 주석으로 박아 뒀다. 값을 명시하지 않는 것 자체가
의도이므로, 명시하지 않은 이유를 적어 두지 않으면 다음 사람이 "설정이
빠졌네" 로 읽는다.

### 가드를 고치지 않은 이유

번호를 구간이나 집합으로 들면 순서 뒤바뀜을 견딜 수 있다. 하지만
`seq` 는 **DynamoDB 에 저장되는 blob 의 일부**라 형식이 바뀌면 마이그레이션이
필요하고, 진행 중인 윈도우가 두 형식으로 갈린다.

지금 필요한 것은 "동시 처리를 견디는 가드" 가 아니라 **"밀리지 않는 집계기"**
이고, 그건 샤드로 풀린다. **필요하지 않은 일반화를 위해 저장 형식을 바꾸지
않는다.**

대신 **버릴 때 조용하지 않게** 했다. `cur > hi`(순서 뒤바뀜)와
`cur == hi`(정상 재시도)를 구분해, 앞의 경우에만 stderr 로 경고를 낸다.
되돌릴 방법은 없지만 CloudWatch 로그(`/aws/lambda/o2-agg`)에 흔적이 남아
"왜 건수가 줄었지" 를 추적할 수 있다.

`warm/tests/test_sequence_guard.py` 가 이 성질 전부를 고정한다. 그중
`test_out_of_order_batch_on_the_same_shard_is_silently_lost` 는 **결함을
고정한 것이 아니라 제약을 고정한 것**이다 — 그 시험이 깨진다면 누군가
가드를 고쳤다는 뜻이고, 그때는 이 결정의 제약도 같이 풀렸는지 봐야 한다.

### 아직 안 한 것

**샤드를 실제로 늘리지는 않았다.** 지연은 부하 시험 중에만 관측됐고
(M-015 — 48시간 중 42시간이 6시간당 5건), 샤드는 상시 비용이다. 부하가
상시화되거나 데모에서 90초 루프를 실제로 돌릴 때 올린다.

올릴 때 확인할 것 — `maximum_batching_window_in_seconds = 2` 는 성능 설정이
아니라 **서로 다른 서비스의 이벤트를 같은 10초 윈도우에서 만나게 하는 값**
이다. 샤드를 늘려도 이 값은 건드리지 않는다.

---

## D-064. 파드별 CPU 는 조임 비율이 아니라 사용량으로 본다 — `limits.cpu` 를 안 건다

**맥락** — 적합성 검토 F-6 이 이렇게 권고했다.

> 정상 파드에도 넉넉한 `limits.cpu` 를 걸어 분모를 만드는 쪽을 권한다 —
> 연출이 아니라 정상 운영 설정이다.

이유는 cAdvisor 가 **CPU limit(= CFS 쿼터)이 있어야** 스로틀링 지표를
만들기 때문이다. limit 이 없으면 `kubernetes.cpu.cfs.*` 가 아예 안 생기고,
S2 재분석에서 *"정상 대비 몇 배로 조이고 있나"* 를 계산할 분모가 없다.

`O2-live-deploy` 에 요청했고, **거절됐다. 그 판단을 받아들인다.**

### 왜 거절이 맞는가

그 저장소는 **모든 디플로이먼트에서 CPU limit 을 일부러 안 건다.** 서비스마다
사유가 다르게 적혀 있다.

| 서비스 | 매니페스트 주석 |
|---|---|
| `api` | 스로틀링이 지연을 만든다 |
| `chat-gateway` | 스로틀링이 팬아웃 지연을 만든다 |
| `order-worker` | 스로틀링이 처리량을 깎는다 |
| `mediamtx` | 스로틀링이 세그먼트 지연을 만든다 |

**응답 지연을 재는 시스템에서 지연을 만드는 설정을 켜는 것은 앞뒤가 안
맞는다.** 우리가 "넉넉하게 걸면 안 걸린다" 고 제안했지만, 그건 **관측을
위해 운영 설정을 바꾸는 것**이고 방향이 거꾸로다. 지표가 시스템에 맞춰야지
시스템이 지표에 맞출 일이 아니다.

`loadtest/run.sh` 의 판정 기준도 이 전제 위에 서 있다 — *"CPU limit 을
일부러 안 걸었으므로 판정이 깔끔하다"*. limit 을 걸면 "CPU 포화" 와
"쿼터에 막힘" 이 섞여 그 판정이 흐려진다.

### 결정

**파드별 CPU 는 `kubernetes.cpu.usage.total{...} by {pod_name}` 으로 본다.**
조임 비율(`kubernetes.cpu.cfs.throttled.periods / .periods`)은 이 클러스터의
앱 네임스페이스에서 **영구히 비어 있는 것으로 확정한다.**

- `05-datadog/dashboard_infra.tf` 의 CFS 위젯은 남긴다. `$kube_namespace` 가
  템플릿 변수라 `keda` 로 바꾸면 값이 나오고(그쪽 Helm 차트가 limit 을 건다),
  "왜 비어 있나" 의 답이 위젯 옆에 있는 편이 낫다. **note 를 "아직 안 됐다"
  에서 "안 하기로 했다" 로 고쳤다**
- `latency_p95_pod_outlier` Monitor 의 진단 안내에서 조임 비율을 뺀다.
  없는 것을 보라고 하면 안 된다

### 잃는 것 — 정확히 하나다

*"이 파드가 CPU 쿼터에 막혀 있다"* 를 **직접** 말하는 신호가 없다.
사용량으로는 *"이 파드가 CPU 를 많이 쓴다"* 까지만 말할 수 있다.

**S2 에는 충분하다.** 재분석이 답해야 하는 질문은 "어느 파드가 느린가" 이고,
그건 `latency_p95 by {pod_name}` 이 답한다(D-057). CPU 는 그 다음 질문인
"왜" 의 후보 하나일 뿐이고, 사용량이 정상 파드와 다르면 그것으로 지목된다.

그리고 S2 실험이 파드 하나를 조일 때는 **그 파드에만** limit 이 걸리므로
CFS 시계열이 하나 생긴다. 비율은 못 내지만 **시계열이 존재한다는 사실
자체가 그 파드를 지목한다.** 분모가 없는 것이 치명적이지 않다.

### 다시 볼 때

- `api` 의 uvicorn 워커가 여럿이 되면 — CPU 특성이 바뀐다
- HPA 나 VPA 를 붙이면 — 그쪽이 limit 을 요구할 수 있다
- 조임이 실제로 의심되는 사고가 나면 — 그때 한 파드에만 임시로 걸어 잰다.
  상시 설정으로 만들지 않는다
