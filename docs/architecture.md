...
| 바뀐 것 | v1.1 | v1.2 |
|---|---|---|
| D-15 채팅 소비 범위 | 팬아웃 소비자만 고려, Valkey Pub/Sub만 | **팬아웃은 Valkey 유지, 장애 분석은 Chat Gateway에서 전용 SQS로 직접 분기(D-047)** |

상세 처리 규칙은 `docs/chat-incident-candidate.md`, 입출력 스키마는
`docs/contracts.md` 5.6·5.7을 원본으로 삼는다.
...
| D-15 채팅 다중 소비 | 미결정 | **v1.1 당시 확정 — 팬아웃에 한정. 장애 분석 분기는 v1.2 D-047이 대체** |
...
| D-15 | 채팅 이벤트 소비 | **실시간 팬아웃은 Valkey Pub/Sub, 장애 분석은 Chat Gateway → 전용 SQS** | **v1.2 교체(D-047)** |
...
같은 보존 이벤트를 모더레이션·아카이브 등 여러 독립 소비자가 재사용해야 할 때만
Valkey Streams 또는 SNS → 다중 SQS를 재검토한다. 현재 AIOps 장애 신호는 D-047에 따라
Chat Gateway가 전용 SQS로 직접 분기하므로 이 재검토 조건에 해당하지 않는다.
...
| ~~D-15~~ | ~~채팅 이벤트 소비~~ | **v1.2 교체 — 팬아웃은 Valkey, 장애 분석은 전용 SQS(D-047)** | v1.2 |
