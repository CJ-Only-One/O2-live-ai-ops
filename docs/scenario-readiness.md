...
| 인입 급증 알림 | S1 진입 Monitor는 `scenario_alerts.tf`의 `s1_chat_fanout_volume`(`[O2][S1] 채팅 팬아웃 총량`, 22078626, `last_1m`, `role:entry`)이고 mapping·allowlist에도 등록됐다. 다만 message에 `@webhook-o2-incident-entry`가 없어 현재는 Source Adapter로 들어가지 않는다. 옛 `chat_ingest_surge`는 사람용 `[O2][보류]`다 | **고쳐야** |

### 2.3 S2 — 느린 파드 / 자기 교정 게이트

| 요구 | 현재 | 판정 |
|---|---|---|
| canary Deployment (같은 Service, CPU·probe 관측창만 변경) | `loadtest/s2-canary.sh`가 클러스터의 현재 main Deployment를 원본으로 읽고 CPU 상한과 readiness/liveness 허용치를 입력받는다. main과 Deployment selector는 분리하고 Service 멤버십 라벨만 공유한다. server-side dry-run과 200 RPS 실부하 재검증을 완료했다 | **구현·실측됨** |
...
| **목업 PG 스텁** | `apps/api/app/services/payment.py`가 주문 예약 뒤 `cfg:pg:*` 지연·결정론적 실패를 적용하고 `payment.process`를 발행한다. PG 실패 시 재고·멱등키를 보상한다(D-078). Runbook 승격 증거에서 같은 주입의 PG-A/PG-B 결과가 기록됐다 | **구현·실측됨** |
| **`cfg:pg:*` 노브** | `/api/admin/pg-stub`이 별도 admin key로 `delay_ms`·`fail_rate`를 함께 SET·DEL한다. 단위 테스트와 Runbook 승격 실측에 사용됐다 | **구현·실측됨** |
| S3 Datadog 후속 증거 Monitor | state ID `22078625`(PG p95)를 `CORROBORATING`, `22078627`(결제 실패율)을 `CONTEXT`로 map·allowlist에 넣고 둘 다 Incident webhook으로 보냈다. 2026-08-28 live apply는 끝났고, Chat→두 Monitor가 같은 PG 인시던트로 병합되는 E2E는 아직이다 | **적용됨 · E2E 미검증** |
...
| 1차 실행: active Runbook 없음 → 실패 보고 | 11-B가 `runbook_status`를 하류로 넘기고, 13-A가 `active`·`experiment`가 아니면 `MANUAL_REQUIRED` + `NO_ACTIVE_RUNBOOK`을 내 `ESCALATED`로 끝낸다. `dify/test_no_candidate_action.py`가 DSL 원본을 검증한다. 저장소 DSL에는 구현됐지만 게시 graph의 동일 분기 실 E2E는 확인되지 않았다 | **구현됨 · live 미검증** |
| 사람 해결 사례 → verified History | `06-agent/scripts/verify.py`가 `human_fixed`와 원인을 사람이 확정한 사례만 verified로 만든다. 이 경로와 verified 사례의 Runbook 승격 사용은 확인됐고, 없는 것은 1차 재촬영용 격리 데이터셋·벡터 인덱스다 | **있음 · 반복 격리 없음** |
...
| PG-B 상태·전환·원복 제어면 | `/api/admin/pg-provider-switch`가 PG-B ready 확인 후 PG-A→PG-B 전환하고, PG-A 주입 해제 뒤에만 원복한다. 전환·원복은 멱등 처리하고 통합 테스트와 Runbook 승격 증거에서 PG-B 성공을 확인했다 | **구현·검증됨** |
| PG-A→PG-B Action Handler | `switch_pg_provider`가 `L3`로 등록돼 Slack 승인 경로를 타고, 저장소 DSL은 `$PG_PROVIDER_SWITCH_URL`과 admin key 환경변수를 참조한다. 게시 graph는 REAL 호출까지 확인됐지만 URL·key가 Code 노드에 평문이라 비밀값 회전과 재게시가 필요하다 | **있음 · 보안 드리프트 · E2E 부분** |
| `pg_latency_ratio` 집계 | `o2warm/sketch.py`·`metrics.py`에 있고 `payment.process`가 `pg_latency_ms`를 발행한다. PG-A/PG-B 지연 증거는 있으나 `pg_latency_ratio` 자체의 별도 실측 행은 없다 | **부분 실측** |
...
| 요구 | 현재 | 판정 |
|---|---|---|
| 데모용 `last_1m-2m` 모니터 | `05-datadog/terraform.tfvars`의 `scenario_entry_window_minutes`·`scenario_early_window_minutes`가 둘 다 `1`이라 S1·S2·S3 진입 Monitor가 모두 `last_1m`이다. S3를 `last_5m`로 되돌리는 것은 미결정 사항이지 현재 설정 오류가 아니다 | **있음 · 결정 필요** |
...
5. **S3 주입 프로필의 남은 검증** — Runbook 승격에는 `delay_ms=1200`,
   `fail_rate=0.9`를 썼다. 같은 값과 1차에 기록한 주문 RPS에서 읽기 경로 생존·오진 여부·
   필요 VU 수를 1차·2차 E2E 기록으로 남긴다.
6. **S3 반복 시연 격리** — verified History와 active Runbook 승격 경로는 있다.
   공유 append-only History를 지우지 않고 1차를 재촬영할 격리 데이터셋·벡터 인덱스만 만든다.
7. **남은 효과 실측** — S1 제한 강도별 차단률·전파 p95, S2 자동 격리·최종 원복,
   S3 Chat+Datadog 병합부터 최종 `RESOLVED`까지를 `measurements.md`에 남긴다.
...
1. **시나리오 Monitor 라우팅 정리** — 옛 `@webhook-o2-dify`는 S1 전파 p95와
   S2 꼬리 지연에 남아 있다. S1 팬아웃 PRIMARY와 차단률은 incident webhook이 없고,
   S3 두 Monitor만 새 Incident 경로를 쓴다. S1·S2를 전환한 뒤 옛 직결 경로를 없앤다.
...
| 1 | S1 PRIMARY·증거 Monitor를 Incident 경로로 전환하고 병합 범위를 보정 | 현재 S1은 Source Adapter에 진입하지 않고 방송 간 오병합 위험이 있다 |
| 2 | S1 전파 p95를 Agent 검증 입력에 연결하고 제한 강도별 효과 실측 | Monitor는 있지만 Dify 검증값은 `None`이다 |
| 3 | S2 진단 입력을 회차별로 나누고 자동 격리·원복 E2E | 현재 1·2차가 같은 입력을 받아 자기 교정이 멈춘다 |
| 4 | S3 비밀값 회전 후 환경변수 기반 Dify graph 재게시 | REAL 호출은 있으나 URL·key 평문 배포본이다 |
| 5 | S3 격리 History/Runbook으로 동일 장애 1차·2차 E2E | active Runbook 때문에 공유 데이터로는 1차 실패를 재현할 수 없다 |
| 6 | 남은 효과·오병합·MTTR 실측을 `measurements.md`에 기록 | 구현 사실과 실험 성공을 분리한다 |
| 7 | 녹화 프로필과 모니터 표시 이름 확정 | 동작 검증 뒤 시연 시간만 조정한다 |
...
M-016(파드 축 전제)과 S2 canary/수동 조치, S3 Runbook 승격 증거다. 아래에는 미측정과
부분 실측을 함께 남긴다. 재면 `measurements.md`의 해당 절을 가리키도록 바꾼다.
...
| 채팅 전파 기준값 · 총량 제한 강도별 인입 감소량과 차단률 | M-010은 붕괴 형상만 측정했다. S1 자동 조치 성공·실패 기준은 미측정 |
| ~~CPU 상한 창~~ | 2026-08-26 스윕으로 `125m` 확정(`measurements.md` S2 canary CPU 상한 스윕). 15분 연속 부하에서는 재시작 3회라 **60초 부하에서만 성립**한다 |
| 증설 전후 서비스 p50·p95·p99 · 격리 후 복귀 폭 · 반복 재현율 | 수동 scale/isolate/rollback 효과는 측정했다. 자동 E2E와 반복 재현율은 미측정 |
| 결제 PG 주입 강도(`delay_ms` × 주문 RPS)와 읽기 경로 생존 구간 | `1200ms`·`0.9`에서 PG-A/PG-B 차이는 측정했다. 전체 E2E의 주문 RPS·읽기 생존·오진 여부는 별도 측정 기록이 필요하다 |
| PG-B 상태 확인·전환·원복 시간과 실패 조건 | S3 Failover Runbook·Guardrail 입력 |
| 동일 PG-A 주입에서 PG-A 실패율과 PG-B 성공 이벤트·주문 p95·채팅 불만 감소 | PG-A 실패율/p95와 PG-B 성공/p95는 Runbook 승격 증거에 있다. 채팅 불만 감소와 최종 E2E는 미측정 |
